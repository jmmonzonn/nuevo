        ...
        fetch("http://localhost:3001/accounts/" + store.userAccount.id, {
            ...
            .then(res => {
                if (res.status === 200) {
                    ...
                    actions.setItem("message", `Depósito completado. Saldo actual: ${formatter.format(parseFloat(store.userAccount.ammount))}`)
                    setTimeout(() => {
        ...
        <>
            <div className="absolute h-screen w-11/12 md:w-9/12 left-16 top-11 md:left-64 bg-gray-700  flex flex-col gap-9  place-items-start ">
                <div className=" w-11/12 ">
                    <div className=" bg-gray-800 min-w-full rounded-xl ml-6 py-4 mt-8">
